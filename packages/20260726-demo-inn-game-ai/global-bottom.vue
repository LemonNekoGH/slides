...
/**
 * Copied from AyakaNeko's VueConf 2026 deck.
 * The hue progression provides a shared default that slides can override.
 */
...
function usePoly(number = 16) {
  function getPoints(): Range[] {
    ...
    function applyOverflow(random: number, amount: number) {
      random = random * (1 + amount * 2) - amount
  ...
  function jumpPoints() {
    ...
    points.value = points.value.map((oldPoint) => {
      ...
      for (const newPoint of newPoints) {
        const distance = distance2(oldPoint, newPoint)
...
<template>
  <div>
    ...
    >
      <div
        class="clip bg-gradient-to-r from-[#facc15] to-[#eab308]"
        :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
      />
      <div
        class="clip bg-gradient-to-l from-[#facc15] to-[#eab308]"
        :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
      />
      <div
        class="clip bg-gradient-to-t from-[#facc15] to-[#eab308]"
